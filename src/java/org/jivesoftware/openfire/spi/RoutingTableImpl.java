...
public class RoutingTableImpl extends BasicModule implements RoutingTable, ClusterEventListener {
    ...
    public RoutingTableImpl() {
        ...
        node = JiveGlobals.getXMLProperty(CLUSTER_PROPERTY_NODE);
        ip = JiveGlobals.getXMLProperty(CLUSTER_PROPERTY_IP);
    }
        ...
        if (destination.getAuthToken().isAnonymous()) {
            ...
            if (route.getResource() != null && (!available || added)) {
                ...
                try {
                    ...
                    redisSessionCompent.addUserSession(route, jids);
                    String domain= destination.getAddress().getDomain();
                    String userNode=destination.getAddress().getNode();
                    String serverIP=ip;
                    String presence=destination.getPresence().toXML();
                    String serverNode=node;
                    String hostAddress = "UnknownHost";
                    try {
                        hostAddress= destination.getHostAddress();
                    }catch (UnknownHostException e) {
                        Log.warn("Maybe user login out");
                    }
                    RedisClientSessionEntity entity = new RedisClientSessionEntity(domain, userNode, serverIP, presence, serverNode, hostAddress);
                    redisSessionCompent.addClientSession(route, entity);
                    if (isGroupMember(userNode)) {
                        redisSessionCompent.addOnlineCustomerFlag(userNode);
                    }
                } finally {
                    lock.unlock();
                }
    ...
    public void routePacket(JID jid, Packet packet, boolean fromServer) throws PacketException {
        ...
        try {
            if (jid.getNode() != null) {
                String key = "UserSession:" + jid.toBareJID();
                Jedis jedis = XMPPServer.getInstance().getJedisPool().getResource();
                Set<String> keys = jedis.hkeys(key);
                if (keys != null) {
                    for (String resource : keys) {
                        String full_jid = "ClientSession:" + jid.toBareJID() + ":" + resource;
                        String redis_server_node = jedis.hget(full_jid, "ServerNode");
                        String redis_server_ip = jedis.hget(full_jid, "ServerIP");
                        if (!ip.equals(redis_server_ip) && !node.equals(redis_server_node)) {
                            jedis.publish(redis_server_node + "_" + redis_server_ip, packet.toXML());
                        } else {
     ...
     * @param jid        the recipient of the packet to route.
     * @param packet     the packet to route.
     * @param fromServer true if the packet was created by the server. This packets
    ...
    private boolean routeToRemoteDomain(JID jid, Packet packet,
        ...
        Jedis jedis = XMPPServer.getInstance().getJedisPool().getResource();
        String node = jedis.hget(key, "ServerNode");
        jedis.close();
        if (node != null) {
    ...
    public boolean removeClientRoute(JID route) {
        ...
        redisSessionCompent.removeUserSession(route);
        redisSessionCompent.removeClientSession(route);
        if (isGroupMember(route.getNode())){
            redisSessionCompent.removeOnlineCustomerFlag(route.getNode());
        }
    ...
    public void initialize(XMPPServer server) {
        ...
        redisSessionCompent = server.getRedisSessionCompent();
    ...
    private boolean isGroupMember(String username){
        try {
            return groupManager.getGroup("group1").getMembers().contains(new JID(username + "@" + server.getServerInfo().getXMPPDomain()));
