...
//                replyWellcomeMessage(presence.createCopy());
                if (session != null) {
                ...
                redisSessionCompent.updateClientSessionPresence(presence);
            }
            ...
            redisSessionCompent.userStatusNotify(presence);
        }
        catch (Exception e) {
    ...
    private void replyWellcomeMessage(Presence presence) {
        if (presence.getFrom() == null) {
            return;
        }
        if (localServer.isLocal(presence.getFrom())) {
            // Do nothing if roster service is disabled
            if (!RosterManager.isRosterServiceEnabled()) {
                return;
            }
            // Local updates can simply run through the roster of the local user
            String name = presence.getFrom().getNode();
            try {
                if (name != null && !"".equals(name)) {
                    Map<String,String> map = new HashMap<>();
                    Long time = System.currentTimeMillis();
                    map.put("msg_id",time.toString());
                    map.put("form_jid",presence.getFrom().toFullJID());
                    map.put("form_nick","系统");
                    map.put("form_uid","");
                    map.put("form_icon","");
                    map.put("type","system");
                    map.put("message","");
                    map.put("to_jid",presence.getFrom().toFullJID());
                    map.put("to_uid","");
                    map.put("to_icon","");
                    ...
                    message.setBody(new JSONObject(map).toString());
                    session.deliverRawText(message.toXML());
                }
            ...
            Log.warn("Presence requested from server "
                    ...
                    + " by unknown user: " + presence.getFrom());
        }
    }
	...
	public void initialize(XMPPServer server) {
        ...
        redisSessionCompent = new RedisSessionCompentImpl(server.getJedisPool());
