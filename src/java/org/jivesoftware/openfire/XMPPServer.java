...
import org.jivesoftware.openfire.cluster.ClusterMessageListener;
...
import org.jivesoftware.openfire.session.RedisSessionCompent;
import org.jivesoftware.openfire.session.RedisSessionCompentImpl;
...
public class XMPPServer {
    ...
    private static final String REDIS_ENABLE = "redis.enable";
    private static final String REDIS_POOL_MAXIDLE = "redis.pool.maxIdle";
    private static final String REDIS_POOL_MAXTOAL = "redis.pool.maxTotal";
    ...
    private static final String REDIS_DB_INDEX = "redis.database.index";
    private static final String CLUSTER_PROPERTY_NODE = "clustering.node";
    private static final String CLUSTER_PROPERTY_IP = "clustering.ip";
    private boolean shuttingDown;
    private XMPPServerInfoImpl xmppServerInfo;
    private RedisSessionCompent redisSessionCompent;
...
//                            ((AdminConsolePlugin) pluginManager.getPlugin("admin")).startup();
    ...
    public void start() {
        try {
            ...
            pluginManager.start();
            // connection to Redis service,start cluster
            startCluster();
    ...
    private void startRedisConnection() {
        if (JiveGlobals.getXMLProperty(REDIS_ENABLE, false)) {
            final String node = JiveGlobals.getXMLProperty(CLUSTER_PROPERTY_NODE);
            final String ip = JiveGlobals.getXMLProperty(CLUSTER_PROPERTY_IP);
            setNodeID(NodeID.getInstance(node.getBytes()));
            logger.info("Openfire 初始化连接Redis...");
            // 连接Redis
            JedisPoolConfig jedisPoolConfig = new JedisPoolConfig();
            jedisPoolConfig.setMaxIdle(Integer.parseInt(JiveGlobals.getXMLProperty(REDIS_POOL_MAXIDLE, "200")));
            jedisPoolConfig.setMaxTotal(Integer.parseInt(JiveGlobals.getXMLProperty(REDIS_POOL_MAXTOAL, "10")));
            jedisPoolConfig.setMaxWaitMillis(Integer.parseInt(JiveGlobals.getXMLProperty(REDIS_POOL_MAXWAIT, "1000")));
            jedisPoolConfig.setTestOnBorrow(true);

            String host_name = JiveGlobals.getXMLProperty(REDIS_HOST, "127.0.0.1");
            Integer port = Integer.parseInt(JiveGlobals.getXMLProperty(REDIS_PORT, "6379"));
            String pwd = JiveGlobals.getXMLProperty(REDIS_PASSWORD, null);
            Integer databases_index = JiveGlobals.getXMLProperty(REDIS_DB_INDEX, 0);
            jedisPool = new JedisPool(jedisPoolConfig, host_name, port, 5000, pwd, databases_index);
            new Thread(new Runnable() {
                @Override
                public void run() {
                    Jedis jedis = jedisPool.getResource();
                    jedis.subscribe(new ClusterMessageListener(node + "_" + ip), node + "_" + ip);
                }
            }).start();

            Jedis jedis = jedisPool.getResource();
//            jedis.flushDB();
            jedis.hset("cluster:" + xmppServerInfo.getXMPPDomain() + ":" + node, "info", "{\"start_time\":\""+System.currentTimeMillis()+"\",\"ip\":\""+ip+"\"}");
            jedis.close();
            redisSessionCompent = new RedisSessionCompentImpl(jedisPool);
        } else {
            logger.warn("Openfire中的Redis开关处于关闭状态，即不启动链接Redis");
        }
    }
    ...
    public RedisSessionCompent getRedisSessionCompent() {
        return redisSessionCompent;
    }
}
