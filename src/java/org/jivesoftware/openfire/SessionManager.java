...
public class SessionManager extends BasicModule implements ClusterEventListener/*, ServerItemsProvider, DiscoInfoProvider, DiscoItemsProvider */ {
    ...
    public boolean removeSession(ClientSession session, JID fullJID, boolean anonymous, boolean forceUnavailable) {
        ...
        /*
        try {
            if (fullJID.getNode() != null && fullJID.getNode().trim().length() > 0) {
                JedisPool jedisPool = XMPPServer.getInstance().getJedisPool();
                if (jedisPool != null) {
                    Jedis redis = jedisPool.getResource();
                    String jia_uid = redis.hget("account_linking", fullJID.getNode());
                    if (jia_uid != null) {
                        redis.hdel("account_linking", jia_uid);
                        redis.sadd("idle_account", fullJID.getNode());
//                        redis.rpush();
                    }
                    redis.hdel("account_linking", fullJID.getNode());
                    redis.close();
                }
            }
        } catch (Exception e) {
