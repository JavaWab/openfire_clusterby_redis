...
public class RESTServicePlugin implements Plugin, PropertyEventListener {
    ...
    private String LOAD_USER = "SELECT username FROM ofUser WHERE '10000'<=username AND username <='12000' AND username != 'admin' limit ?,?";
    ...
    public void initializePlugin(PluginManager manager, File pluginDirectory) {
        ...
        Connection con = null;
        try {
            Jedis redis = XMPPServer.getInstance().getJedisPool().getResource();
            String result = redis.hget("System", "inited");
            if (result == null) {
                con = DbConnectionManager.getConnection();
                int index = 1;
                int pageCount = 1000;
                while (true) {
                    PreparedStatement pstmt = con.prepareStatement(LOAD_USER);
                    pstmt.setInt(1, pageCount * (index - 1));
                    pstmt.setInt(2, pageCount * index);
                    ResultSet rset = pstmt.executeQuery();
                    List<String> jid_page = new ArrayList<>();
                    while (rset.next()) {
                        String username = rset.getString("username");
                        redis.sadd("idle_account", username);
                        jid_page.add(username);

                    }
                    int size = jid_page.size();
                    if (size == 0 || size < pageCount) {
                        redis.hset("System", "inited", "true");
                        break;
                    }
                    index++;
                }
