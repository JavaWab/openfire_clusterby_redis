...
public class UserServiceController {
    ...
    public void createUser(UserEntity userEntity) throws ServiceException {
        if (userEntity != null && !userEntity.getUsername().isEmpty()) {
            ...
            try {
                userManager.createUser(userEntity.getUsername(), userEntity.getPassword(), userEntity.getNick(),
                        userEntity.getEmail());
    ...
    public void updateUser(String username, UserEntity userEntity) throws ServiceException {
        if (userEntity != null && !username.isEmpty()) {
            ...
            if (userEntity.getNick() != null) {
                user.setName(userEntity.getNick());
            }
     ...
     * @return
     */
    public GuestAccountEntity getRandomGuest(String uid) throws ServiceException {
        String username = jedis.hget("account_linking", uid);
        System.out.println("getRandomGuest from redis :" + username);
        if (username != null) {
            return new GuestAccountEntity(username + "@" + XMPPServer.getInstance().getServerInfo().getXMPPDomain(), uid);
        } else {
            while (true) {
                username = jedis.spop("idle_account");
                if (username == null || "".equals(username.trim())) {
                    throw new ServiceException("Guest Accounts has been used up", "uid", "GuestAccountError", Response.Status.NOT_FOUND);
                } else {
                    jedis.hset("account_linking", username, uid);
                    jedis.hset("account_linking", uid, username);
                    return new GuestAccountEntity(username + "@" + XMPPServer.getInstance().getServerInfo().getXMPPDomain(), uid);
    ...
    public UserEntity getUserEntity(String username) throws ServiceException {
        UserEntity userEntity = UserUtils.convertUserToUserEntity(getAndCheckUser(username));
        Collection<ClientSession> sessions = sessionManager.getSessions(username);
        userEntity.setStatus(sessions.isEmpty() ? "Offline" : "Online");
        return userEntity;
    }
     ...
     * @throws ServiceException
     */
    public List<Map<String, Object>> getAllGroups() throws ServiceException {
        Collection<Group> groups = GroupManager.getInstance().getGroups();
        List<Map<String, Object>> group_list = new ArrayList<>();
        for (Group group : groups) {
            Map<String, Object> mgp = new HashMap<>();
            mgp.put("group_name", group.getName());
            mgp.put("group_description", group.getDescription());
            mgp.put("group_jid", group.getJID());
        }
        return group_list;
    }
     ...
     * @throws ServiceException
     */
    public Map<String, Object> getGroupsByName(String name) throws ServiceException, GroupNotFoundException {
